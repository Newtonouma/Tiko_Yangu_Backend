...
export class MpesaService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      throw new HttpException('Failed to get M-Pesa access token', HttpStatus.BAD_GATEWAY);
    }
  }

  async stkPush({ amount, phone, accountReference, transactionDesc }: { amount: number; phone: string; accountReference: string; transactionDesc: string; }) {
    ...
    try {
      ...
      if (error.response) {
        ...
        throw new HttpException('M-Pesa STK Push failed', HttpStatus.BAD_GATEWAY);
      }
    }
  }
