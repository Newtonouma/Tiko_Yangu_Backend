...
export class AnalyticsService {
  ...
  async getRevenueAnalytics(
    ...
    const projectedRevenue =
      Math.round((currentMonthRevenue / currentDay) * daysInMonth * 100) / 100;

    // Revenue by month
    const monthlyStats = tickets.reduce(
      (acc, ticket) => {
    ...
    const revenueByEvent = Object.values(eventStats).map((event: any) => ({
      ...
      averageTicketPrice:
        event.ticketsSold > 0
          ? Math.round((event.revenue / event.ticketsSold) * 100) / 100
          : 0,
    }));
  ...
  async getUserAnalytics(): Promise<{
    ...
    const organizerStats = events.reduce(
      (acc, event) => {
    ...
    const averageEventsPerUser =
      totalUsers > 0 ? events.length / totalUsers : 0;
    const averageTicketsPerUser =
      totalUsers > 0 ? tickets.length / totalUsers : 0;
